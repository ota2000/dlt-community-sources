...
- 400/403/404 (auxiliary): declare the skip **at the call site** — `response_actions` with `"action": "ignore"` in rest_api config for declarative resources, `skip_or_raise` (from `_utils`) in custom fetchers. Some APIs return 4xx for valid requests on accounts without certain features (e.g., Yahoo Ads FeedSetService). There is no blanket resource wrapper: every expected skip must be visible where the request is made, mirroring dlt's per-endpoint `response_actions` philosophy
- 401: refresh token and retry (App Store Connect uses per-request JWT via `session.auth`)
- **Primary resources must fail loudly**: raise `PrimaryResourceTerminalError` (request rejected: 4xx other than 408/429, or a well-formed response missing the expected payload) or `PrimaryResourceTransientError` (timeout, rate limit, provider-side job failure, 5xx) — never log-and-skip. Use `primary_error_from_http` to classify an `HTTPError` automatically. These subclasses mix in dlt's `TerminalException` / `TransientException`; dlt wraps resource exceptions (`PipelineStepFailed` / `ResourceExtractionError`), so retry predicates must use `contains_terminal_exception` (walks the chain) rather than `retry_load` (inspects one level). A silent skip means the load "succeeds" with missing data, which downstream consumers cannot detect. This includes API-specific error styles (e.g., TikTok's HTTP 200 + `code != 0`)
- **dlt's `Client.get()/post()` raises inside the call** (`raise_for_status=True` default): the request call itself must be inside the `try` block — a trailing `response.raise_for_status()` after the call is dead code and its except branch is unreachable
