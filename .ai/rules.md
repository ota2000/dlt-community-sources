...
- **Primary resources must fail loudly**: raise `PrimaryResourceTerminalError` (request rejected: 4xx, or a well-formed response missing the expected payload) or `PrimaryResourceTransientError` (timeout, provider-side job failure, 5xx) — never log-and-skip. Use `primary_error_from_http` to classify an `HTTPError` automatically. These subclasses mix in dlt's `TerminalException` / `TransientException`, so `dlt.pipeline.helpers.retry_load` makes correct retry decisions. A silent skip means the load "succeeds" with missing data, which downstream consumers cannot detect. This includes API-specific error styles (e.g., TikTok's HTTP 200 + `code != 0`)
